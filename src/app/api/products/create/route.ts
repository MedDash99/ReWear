...
      sellerId: sellerIdFromClient, // Assuming client sends this; needs careful handling
    } = body;
...
    }
    ...
    // Seller ID Handling:
    // In a real application, you should get the sellerId from the authenticated user's session
    // on the server-side to prevent users from assigning listings to others.
    // For now, if it's coming from the client, validate it.
    const sellerId = parseInt(sellerIdFromClient as string, 10);
    if (isNaN(sellerId)) {
        // This indicates a problem with how sellerId is being sent or derived.
        // You might want to fetch the actual user from a session here.
        console.error("Invalid sellerId received from client:", sellerIdFromClient);
        return NextResponse.json({ message: "Invalid or missing seller information." }, { status: 400 });
    }
